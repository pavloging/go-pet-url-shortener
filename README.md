...
# Не запускаются тесты random_test.go

1. В тестировании нужно предполагать все варианты и нулевые значения и меньше нуля
2. Тесты падают из-за того, что при быстрых последовательных вызовах функции `time.Now().UnixNano()` может возвращать одинаковые значения т.к это наносекунды

Вывод: Используйте random.go который написан в этом проекте.

# Создание и запуск HTTP сервера
Это очень важный момент на нашем пути - запуск сервера. После запуска, необходимо протестировать нашу систему, отправить запрос на сервер и посмотреть какой будет ответ.
Чтобы это сделать мы скачаем Postman на компьютер, далее нужно пройти авторизацию. Дальше добавить коллекцию и создать POST запрос, указать url, а так же перейти в Body и написать JSON:
```json
{
	"url": "https://go.dev/"
}
...
![Прикрепил фото, чтобы было легче разобраться:](postman-example.png)

# Проблема при генирации Mock
1. Обновитесь до последней версии: `go install github.com/vektra/mockery/v2@latest`
2. Обновите `go get -u golang.org/x/tools`
3. Явно укажите пакет, в котором находится интерфейс, при генерации моков `mockery --name=URLSaver --dir=internal/http-server/handlers/url/save --output=internal/http-server/handlers/url/save/mocks`

После выполнении всех шагов, mock должен быть создан

# Тесты

### Пирамида тестирования
Есть негласное правило распределения тестов (так должно работать в идеальном мире). Суть его проста: более быстрые и маленькие тесты должны преобладать, так как они быстро разрабатываются и используются часто, во многих местах => дешево делаем приложение стабильным.

Обычно распределение тестов выглядит так:

1. Unit-тесты (70%)
Тестирование отдельных функций или методов в изоляции.

Примеры:
- Тестирование функции random.NewRandomString.
- Тестирование валидации URL.

2. Integration-тесты (20%)
Тестирование взаимодействия между несколькими компонентами.

Примеры:
- Тестирование работы с базой данных (например, SaveURL и GetURL).
- Тестирование взаимодействия с внешним API.

3. Service-тесты (5%)
Тестирование работы сервиса в целом, но без запуска всего приложения.

Примеры:
- Тестирование HTTP-обработчиков (например, /save и /redirect).
- Тестирование обработки ошибок в API (например, возврат 404, если URL не найден).

4. End-to-End тесты (5%)
Тестирование полного цикла работы приложения, включая все зависимости.

Примеры:
- Полный цикл: создание короткого URL, его сохранение и перенаправление.
- Тестирование сценария, когда пользователь создаёт короткий URL и переходит по нему.

### Функциональные тесты
Функциональные тесты проверяют, соответствует ли поведение системы её функциональным требованиям. Они могут находиться на разных уровнях пирамиды:

Unit-тесты: Проверка функциональности отдельных функций.
Integration-тесты: Проверка функциональности взаимодействия компонентов.
Service-тесты: Проверка функциональности API.
End-to-End тесты: Проверка функциональности всего приложения.

# Авторизация
Мы не будем хранить user и password в открытом для пользования yaml файле и сохраним данные в .env файл.

# Для генирации mocks в redirect
Выполним команду:
